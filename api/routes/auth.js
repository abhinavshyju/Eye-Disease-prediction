...
const { User, Password } = require("../model");
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        try {
            const token = crypto.randomBytes(16).toString('hex')
            await User.update({ session_token: token }, { where: { email: user.email } })
            res.json({ "message": "User logged in", "user": { ...user.toJSON(), session_token: token } })
        } catch (error_1) {
...
router.post("/setpass", async (req, res) => {
    const reqbody = req.body;
    try {
        const user = await User.findOne({ where: { session_token: reqbody.token } })
...
router.post("/updatepass", async (req, res) => {
    ...
    try {
        const user = await User.findOne({ where: { session_token: token } })
        if (user) {
            await Password.update({ password: reqbody.data.password }, { where: { user_id: user.id } })
